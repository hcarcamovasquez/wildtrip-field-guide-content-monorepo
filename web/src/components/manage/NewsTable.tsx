...
export default function NewsTable({ onEdit, onPreview, currentUserId, canDelete = false }: NewsTableProps) {
  ...
  const fetchNews = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/manage/news?${params}`)
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/manage/news/${deleteId}`, {
        method: 'DELETE',
      })

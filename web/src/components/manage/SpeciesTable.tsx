...
import type { SpeciesWithBase } from '@/lib/private/repositories/ManageSpeciesRepository'
import { CONSERVATION_STATUSES, getConservationStatus } from '@/lib/utils/conservation-status'
...
export default function SpeciesTable({ canDelete = false }: SpeciesTableProps) {
  ...
  const fetchSpecies = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/manage/species?${params}`)
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/manage/species/${deleteId}`, {
        method: 'DELETE',
      })

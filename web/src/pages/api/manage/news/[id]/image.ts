...
import { canManageNews, type Role } from '@/lib/utils/permissions.ts'
...
export const POST: APIRoute = async ({ request, params, locals }) => {
  ...
  // Check if news exists and lock status
  const checkResult = await ManageNewsRepository.checkLockForImageUpload(parseInt(id), user.id)
  ...
  if (checkResult.lockedByAnother) {
    return new Response(JSON.stringify({ error: 'Esta noticia está siendo editada por otro usuario' }), {
      status: 423,
      headers: { 'Content-Type': 'application/json' },
    })
  }
  ...
  try {
    ...
    // Save to media gallery and update news
    await ManageNewsRepository.uploadFeaturedImage(parseInt(id), url, {
      filename: key,
